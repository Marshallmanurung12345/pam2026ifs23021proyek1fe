    ...
    fun login(username: String, password: String) {
        viewModelScope.launch {
            ...
            repo.login(username, password).fold(
                ...
                onFailure = { e -> _uiState.value = AuthUiState(error = e.message ?: "Login gagal") }
            )
        }
    }
    ...
    fun register(name: String, username: String, password: String) {
        viewModelScope.launch {
            ...
            repo.register(name, username, password).fold(
                onSuccess = { response ->
                    _uiState.value = AuthUiState(successMessage = response.message ?: "Registrasi berhasil!")
                },
                onFailure = { e -> _uiState.value = AuthUiState(error = e.message ?: "Registrasi gagal") }
            )
        }
    }

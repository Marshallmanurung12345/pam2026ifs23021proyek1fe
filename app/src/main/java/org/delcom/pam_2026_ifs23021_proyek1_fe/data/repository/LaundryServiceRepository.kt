...
class LaundryServiceRepository @Inject constructor(
    ...
    suspend fun create(
        ...
        when {
            ...
            r.code() == 401 || r.code() == 403 -> Result.failure(TokenExpiredException())
            else -> Result.failure(Exception("Gagal membuat layanan (${r.code()})"))
        }
    } catch (e: Exception) {
        Result.failure(e)
    }
    ...
    suspend fun update(
        ...
        when {
            r.isSuccessful -> Result.success(r.body() ?: BaseResponse(message = "Berhasil"))
            r.code() == 401 || r.code() == 403 -> Result.failure(TokenExpiredException())
            else -> Result.failure(Exception("Gagal memperbarui layanan (${r.code()})"))
        }
    } catch (e: Exception) {
        Result.failure(e)
    }
    ...
    suspend fun delete(token: String, id: String): Result<BaseResponse> = try {
        ...
        when {
            r.isSuccessful -> Result.success(r.body() ?: BaseResponse(message = "Berhasil"))
            r.code() == 401 || r.code() == 403 -> Result.failure(TokenExpiredException())
            else -> Result.failure(Exception("Gagal menghapus layanan (${r.code()})"))
        }
    } catch (e: Exception) {
        Result.failure(e)
    }
